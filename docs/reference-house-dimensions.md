...
The driveway-facing gable (`front-barn`, maximum Z) also has triangular glazing. The two bedroom windows below become glazed terrace doors, retaining their widths and positions, with thresholds aligned to the flat garage roof surface. The garage terrace has a 1.10 m guard of smoked glass and black profiles on its three exposed edges; the edge against the house stays open. Door glazing and roof-segment terrace settings survive export/import and saved-project reloads. A one-time adaptation upgrades existing fitted studies while retaining subsequent edits.
...
The resulting ground envelope is 11.19 × 18.31 m; the upper envelope is 11.28 × 15.14 m. The exterior inherits Zielonki's dark timber facade and standing-seam metal roof finish. Following the user's 9 September correction, the main gable ends at the upper-floor wall. A shorter perpendicular gable meets it at the main ridge, forming valleys at the side junction. The exposed garage has a separate flat membrane roof at its ceiling level, approximately 7.58 × 3.17 m before its 0.10 m edge extension. Both gable pitches are fitted together toward a maximum ground-to-ridge measurement of 8.90 m across their centres, constrained to the MPZP range of 37–45 degrees. The height tool's terrain datum is a model measurement, not a legal height assessment; other placements or elevations can exceed the target when the minimum pitch binds. Eaves and provisional 2.80 m room heights remain unchanged; subsequent roof edits are preserved. See [the zoning evidence](../knowledge-bank/zielonki/ZONING.md) for the applicable clause and the unconfirmed planning status of a flat garage terrace.

The shorter roof's panels and standing seams are cut to the surface of the long roof at the valley. The long roof is also cut out inside the joined wing, so its slope does not intrude into the open living room. There is no projecting end overhang at that junction. Following the user's further correction, the bedroom and landing partitions along the living void are recessed 1.56 m to the meeting of the roof ridges. The whole wall and its vertical extension share this plane, retaining their thickness and finish. Attached walls and the slab opening follow the new position; a short return at the landing keeps the bathroom partition in place. The parents' bedroom now measures 3.59 × 4.50 m and the living void 7.09 × 7.35 m (52.1115 m²). This one-time saved-project upgrade preserves later edits. Upper wall extensions follow the upper storey in exploded views. The roof cuts are part of the mesh geometry, including shadows and exploded views.

The Zielonki project card continues the saved project. Loading a fitted project with the former continuous garage gable or original three-roof layout applies the corrected gable junction and separate garage cap, retaining rooms, placement and existing finishes. This upgrade runs once and advances the revision so old proposals become stale. Source notes live on the building so site knowledge updates cannot remove them.

The former `Sheltered L-courtyard terrace` is fitted to the inner corner of the actual ground-floor envelope and renamed `Taras przy budynku`. Its 3.51 × 10.56 m footprint (37.07 m²) meets both courtyard facades and uses the house's current position and rotation. The existing terrace surface finish is retained. Saved fitted studies receive this correction once; subsequent terrace edits remain unchanged.

The subsequent ICON glazing adaptation follows the user's two facade photos. The projecting living gable (`rear-barn`, maximum X) has a large glazed roof profile aligned with both rectangular windows below. The adjacent kitchen-side gable (`front-barn`, minimum Z) has asymmetric glazing from the outer edge towards the ridge, aligned with the north-facing ground-floor opening. The original west-facing kitchen window remains at the corner; this interpretation uses the adjacent gable, not a new gable directly above that window. Both profiles follow the existing roof pitch and retain a 0.28 m vertical cladding margin below the slopes. Black frame divisions align across the upper and lower glazing.

The former mezzanine floor initially becomes a 5.53 × 7.35 m void (40.6455 m²) above the living room, enlarged to 7.09 × 7.35 m by the subsequent ridge-aligned partition change described above. Its separate upper-floor room entry and access door are removed; the stair opening and other floors remain. Flat ceiling finishes over the living area are removed, leaving the roof as the upper enclosure over the void. Floor-area metrics exclude slab openings. The reference-house source model stays unchanged. This adaptation migrates fitted saved copies once on opening, records its source note, and preserves later edits. Glazing is persisted on roof segments and rendered as a real cutout in the gable cladding; analytical garden sunlight still uses coarse opaque building envelopes and is not an interior daylight simulation.
