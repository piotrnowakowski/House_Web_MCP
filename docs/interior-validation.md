...
| --- | --- | --- |
| `npm test` | **293 passed**, 37 files | Existing project/domain/persistence/WebMCP suites plus geometry, layout, furniture, 65 asset/floor checks and 5 expansion/height-configuration cases. |
| `npm run test:interior` | **12 scenarios passed** | Desktop/mobile height selection, catalogue expansion, dimensions modal, room split/merge/openings/finishes, three mobile viewport scenarios, WebMCP Apply, GLB recovery and project outputs. |
...
| Deployment asset audit | **192/192 URLs passed** | All desktop/mobile GLBs and PNG thumbnails checked as binary assets under the nested base path. Real production JSON import loads LACK desktop/mobile GLB with HTTP 200; no page exceptions in either viewport. |
...
The dimensions checks cover the pencil trigger, catalogue minimum, Cancel/Escape and focus return, rejected floor overflow, a single history entry, actual scaled GLB bounds, undo/redo and autosave reload at 1280×800 and 390×844. The modal screenshots were visually reviewed: [desktop](../output/interior-editor/dimensions-1280.png), [mobile](../output/interior-editor/dimensions-390.png).

The expansion checks place and reload all 32 requested additions, validate all eight standard/tall pairs, reject insufficient physical/assembly height, preserve installed furniture movement, reject locked changes and undersizing, and accept STOENSE under tables. Desktop/mobile browser flows place standard PLATSA, switch to the actual 241 cm assembly, check one history entry and the new dimension minimum, undo/redo and reload. Evidence: [desktop selector](../output/interior-editor/height-variant-desktop.png), [mobile selector](../output/interior-editor/height-variant-mobile.png), [mobile catalogue](../output/interior-editor/expanded-catalogue-mobile.png). The changed code uses one catalogue-backed variant operation and the existing command/history pipeline; no additional state store or dependencies were introduced.
...
All 64 configurations have a desktop GLB, lighter mobile GLB, PNG thumbnail, placement envelope and manifest record. Maximum desktop geometry is 12,832 triangles; maximum mobile geometry is 4,452 triangles. All 128 GLBs total 34,663,240 bytes; the largest is 1,328,448 bytes. Painted furniture uses a subtle original paint map; fabric, jute, wood and mesh have their own original maps. Assets load on demand. HEKTAR overall width and LINDBYN depth are labelled planning estimates; all mesh bounds are checked against their declared dimensions. Source and licence details are in the [asset audit](ikea-assets.md).

The original [contact sheet](../output/interior-editor/asset-contact-sheet.png) and new [40-configuration contact sheet](../output/interior-editor/expansion-contact-sheet.png) were visually reviewed. The UI was also checked with the furnished Zielonki house. Default mounting elevation, bed-frame dimensions and carcass/assembly distinctions are in the product manifest. Two historical/redirecting Polish references remain explicitly marked.

## Performance and remaining device checks

The production audit used Chrome 152 on Windows with Intel Arc graphics. On the furnished ground floor with **21 objects**, the stationary rendering sample measured **57.7 fps desktop / 60.0 fps mobile viewport**, with 16.7 ms median and 17.0 / 16.8 ms p95 frame intervals. These are requestAnimationFrame samples of a rendered scene on this desktop, not a phone navigation benchmark. Raw environment, asset requests and frame samples are in [production-audit.json](../output/interior-editor/production-audit.json).
