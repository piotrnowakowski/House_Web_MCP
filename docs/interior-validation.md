...
| --- | --- | --- |
| `npm test` | **248 passed**, 36 files | Existing project/domain/persistence/WebMCP suites plus geometry, layout, furniture and 25 asset/floor checks. |
| `npm run test:interior` | **10 scenarios passed** | Desktop editing, dimensions modal at desktop/mobile sizes, room split/merge/openings/finishes, three mobile viewport scenarios, WebMCP Apply, GLB recovery and project outputs. |
...
The dimensions follow-up checks the pencil trigger, catalogue minimum, Cancel/Escape and focus return, rejected floor overflow, a single history entry, actual scaled GLB bounds, undo/redo and autosave reload at 1280×800 and 390×844. The full run passed nine cases; the tenth passed after correcting its mobile Undo locator to open More first. The modal screenshots were visually reviewed: [desktop](../output/interior-editor/dimensions-1280.png), [mobile](../output/interior-editor/dimensions-390.png). Original GLBs and catalogue dimensions remain unchanged.
