...
| Connected walls | Drag a partition or select a wall → exact endpoints/thickness/height. Translations keep existing horizontal/vertical wall runs straight and preserve right angles. Sliding past an adjoining T-junction reconnects the return segment and room boundaries instead of leaving overlapping walls. Shared vertices and hosted opening offsets update together. Exterior geometry stays fixed; use plot tools to change the envelope. |
