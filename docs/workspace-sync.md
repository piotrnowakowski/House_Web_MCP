...
The publisher preserves the existing IPv4/IPv6 bindings, validates frontend assets and database TLS/authentication, and backs up existing `house_sync_*` tables before the new API runs migrations. For every `--sync-api` release it rebuilds `dist-server/main.mjs` from the validated Git revision, logs its SHA-256 digest and verifies the uploaded bundle has the same digest before building the API image; an older ignored local bundle cannot be reused silently. A restricted custom-format PostgreSQL dump stays in the server's `deploy/mikrus/backups/` and is downloaded to ignored local `tmp/` for off-host recovery. First deployment skips the data backup only if application tables do not yet exist. Restore rehearsal must use a separate disposable database with `pg_restore --no-owner --no-acl`, never the live database.
