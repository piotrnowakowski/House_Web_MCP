...
export function InteriorEditor({ onBack }: { onBack: () => void }) {
  ...
  const pick = (point: Vec2) => {
    ...
    if (mode === 'measure') { setPoints((previous) => previous.length === 1 ? [...previous, point] : [point]); return }
    if (pending) {
  ...
  return <main className='interior-editor' aria-label='House interior editor'>
    ...
    <div className='interior-workspace'>
      <section className={`interior-stage ${pending ? 'is-placing' : ''} ${mode === 'measure' ? 'is-measuring' : ''}`} aria-label='Interior floor view'>
        ...
        {(pending || mode === 'measure') && <div className='interior-tool-prompt' role='status'><span>{pending ? `Click the floor to place ${interiorCatalog.find((v) => v.id === pending)?.name.toLowerCase()}` : points.length === 0 ? 'Click near an object or boundary · Alt for free placement' : points.length === 1 ? 'Click the second point · Drag either point to adjust' : `Distance: ${Math.hypot(points[1].x - points[0].x, points[1].z - points[0].z).toFixed(2)} m · Drag either point to adjust · Alt for free placement`}</span><button onClick={clearTool}>Done</button></div>}
      </section>
