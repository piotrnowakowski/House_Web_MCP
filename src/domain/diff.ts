...
const entries = (project: ProjectV2): Array<[ChangeKind, Entry[]]> => {
  ...
  return [
    ...
    ['building', buildings.map((building) => ({ ref: building.ref, fields: pick(building as unknown as Record<string, unknown>, ['name', 'kind', 'designStatus', 'architecturalStyle', 'garageMode', 'position', 'rotationDegrees']) }))],
    ['storey', buildings.flatMap((building) => building.storeys.map((storey) => ({ ref: storey.ref, fields: pick(storey as unknown as Record<string, unknown>, ['name', 'level', 'elevationM', 'clearHeightM', 'kneeWallHeightM', 'baseSlabRef', 'topBoundaryRef', 'wallRefs', 'spaceRefs', 'platformRefs', 'ceilingFinishRefs']) })))],
    ['slab', buildings.flatMap((building) => building.slabs.map((slab) => ({ ref: slab.ref, fields: pick(slab as unknown as Record<string, unknown>, ['footprint', 'holes', 'topElevationM', 'thicknessM', 'edgeColorHex', 'locked']) })))],
    ['wall', buildings.flatMap((building) => building.walls.map((wall) => ({ ref: wall.ref, fields: pick(wall as unknown as Record<string, unknown>, ['start', 'end', 'thicknessM', 'baseElevationM', 'heightM', 'finish', 'faceFinishes', 'locked']) })))],
    ['opening', buildings.flatMap((building) => building.walls.flatMap((wall) => wall.openings.map((opening) => ({ ref: opening.ref, fields: pick(opening as unknown as Record<string, unknown>, ['kind', 'glazed', 'wallRef', 'offsetM', 'widthM', 'heightM', 'sillM', 'hinge', 'swing']) }))))],
    ['space', buildings.flatMap((building) => building.spaces.map((space) => ({ ref: space.ref, fields: pick(space as unknown as Record<string, unknown>, ['name', 'usage', 'boundary', 'baseSlabRef', 'topBoundaryRef', 'floorFinish', 'ceilingFinish', 'locked']) })))],
    ...
    ['roof-segment', buildings.flatMap((building) => building.roof.segments.map((segment) => ({ ref: segment.ref, fields: pick(segment as unknown as Record<string, unknown>, ['footprint', 'storeyRef', 'spaceRef', 'baseElevationM', 'type', 'pitchDegrees', 'overhangM', 'ridgeDirection', 'finish', 'gableWallFinishes', 'gableGlazing', 'gableFrame', 'terrace', 'canopy']) })))],
    ...
    ['zone', project.landscape.zones.map((zone) => ({ ref: zone.ref, fields: pick(zone as unknown as Record<string, unknown>, ['name', 'kind', 'footprint', 'locked', 'textureId']) }))],
