...
const entries = (project: ProjectV2): Array<[ChangeKind, Entry[]]> => {
  ...
  return [
    ...
    ['roof-segment', buildings.flatMap((building) => building.roof.segments.map((segment) => ({ ref: segment.ref, fields: pick(segment as unknown as Record<string, unknown>, ['footprint', 'storeyRef', 'spaceRef', 'baseElevationM', 'type', 'pitchDegrees', 'overhangM', 'ridgeDirection', 'finish', 'gableWallFinishes', 'gableGlazing']) })))],
