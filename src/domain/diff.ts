...
const entries = (project: ProjectV2): Array<[ChangeKind, Entry[]]> => {
  ...
  return [
    ...
    ['opening', buildings.flatMap((building) => building.walls.flatMap((wall) => wall.openings.map((opening) => ({ ref: opening.ref, fields: pick(opening as unknown as Record<string, unknown>, ['kind', 'glazed', 'wallRef', 'offsetM', 'widthM', 'heightM', 'sillM']) }))))],
