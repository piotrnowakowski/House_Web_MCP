...
const entries = (project: ProjectV2): Array<[ChangeKind, Entry[]]> => {
  ...
  return [
    ...
    ['slab', buildings.flatMap((building) => building.slabs.map((slab) => ({ ref: slab.ref, fields: pick(slab as unknown as Record<string, unknown>, ['footprint', 'holes', 'topElevationM', 'thicknessM', 'locked']) })))],
