...
import type { RoofSegmentModel, Vec2, WallFinish } from '../domain/types'
...
export function GlazedGable({ segment, side, value, finish, selected, ghost }: {
  segment: RoofSegmentModel; side: 'min' | 'max'; value: number; finish: WallFinish; selected: boolean; ghost?: boolean
}) {
  const profile = useMemo(() => gableGlazingProfile(segment, side), [segment, side])
  const geometry = useMemo(() => {
    if (!profile) return null
    const points = (polygon: Vec2[]) => polygon.map((p) => new Vector2(p.x, p.z))
    const wall = new Shape(points(profile.outline))
    wall.holes.push(new Path(points([...profile.opening].reverse())))
    return { wall: new ShapeGeometry(wall), glass: new ShapeGeometry(new Shape(points(profile.opening))) }
  }, [profile])
  useEffect(() => () => { geometry?.wall.dispose(); geometry?.glass.dispose() }, [geometry])
  ...
  const edges = profile.opening.map((a, i) => [a, profile.opening[(i + 1) % profile.opening.length]])
  edges.push(...profile.mullions.map((m) => [{ x: m.x, z: m.bottom }, { x: m.x, z: m.top }]))
