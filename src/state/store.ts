...
export const useStudioStore = create<StudioState>((set, get) => ({
  ...
  replaceProject: (project) => { revokeReport(get().structureReport); set({ project, variants: [], proposals: [], draftChangeSets: [], history: [], structureReport: null, toast: `Loaded ${project.name}.` }) },
  restoreWorkspace: (workspace) => {
    ...
    set({ project: workspace.project, proposals, variants, draftChangeSets: staleDrafts(workspace.draftChangeSets, workspace.project.revision), history: [], structureReport: null, toast: `Loaded ${workspace.project.name} with ${proposals.length} proposal record${proposals.length === 1 ? '' : 's'}.` })
  },
