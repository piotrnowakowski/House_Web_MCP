...
describe('ProjectV2 WebMCP surface', () => {
  ...
  it('lists the three catalogues compactly, including the operation reference', async () => {
    ...
    expect(fixtures.data.map((item: { id: string }) => item.id)).toEqual(['recycling-bins', 'jacuzzi', 'outdoor-kitchen', 'outdoor-dining-set', 'garden-lounge-set', 'slatted-bench', 'sun-lounger', 'cantilever-parasol', 'raised-bed-2x1', 'tomato-row', 'potato-row', 'cucumber-trellis'])
