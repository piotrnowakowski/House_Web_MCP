...
it('injects credentials only upstream and refuses arbitrary routes',async()=>{const upstream=vi.fn().mockResolvedValue({status:200,headers:new Headers({'content-type':'application/json'}),text:async()=>'[]'});vi.stubGlobal('fetch',upstream);expect((await call('projects')).body).toBe('[]');expect(upstream.mock.calls[0][1].headers.Authorization).toMatch(/^Bearer /);expect((await call('../health')).statusCode).toBe(404);expect(upstream).toHaveBeenCalledTimes(1)})

it('turns the deployed SPA fallback into an unavailable API response',async()=>{vi.stubGlobal('fetch',vi.fn().mockResolvedValue({status:200,headers:new Headers({'content-type':'text/html'}),text:async()=>'<html>'}));const r=await call('projects');expect(r.statusCode).toBe(503);expect(JSON.parse(r.body).error).toContain('not deployed')})
